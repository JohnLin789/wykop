...
export default function EditPostForm({ initialData }: EditPostFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      await postsApi.update(params.id as string, { title, content, category });
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>更新你的帖子内容</CardDescription>
      </CardHeader>
      <CardContent>
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            <div className="flex items-center justify-between mb-2">
              ...
              <span className={`text-xs ${
                titleLength > maxTitle * 0.9 ? 'text-destructive' : 'text-muted-foreground'
              }`}>
          ...
          <div>
            <div className="flex items-center justify-between mb-2">
              ...
              <span className={`text-xs ${
                contentLength > maxContent * 0.9 ? 'text-destructive' : 'text-muted-foreground'
              }`}>
